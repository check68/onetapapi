...
function indicators(){
	...
	for(active_bind in active_binds){
		...
		Render.String(screen_size[0] / 2 - right_shift + 1, screen_size[1] / 2 + binds_offset, +!betterScopeActive, name, [0, 0, 0, 125], 3);
		Render.String(screen_size[0] / 2 - right_shift, screen_size[1] / 2 + binds_offset, +!betterScopeActive, name, color, 3);
...
var script_items = {
	...
	"better_scope": [
		...
		{"name": "Better scope width", "type": "slider", "min": 0, "max": 200},
...
function drawMenu(){
	...
	if(UI.IsCursorInBox(x + width - 14 - text_size2[0], y + height - 27, text_size2[0], text_size2[1] + 1)){
		hint_text = "Discord: Sleebu#0448\nTelegram: @zxsleebu\nInstagram: @zxsleebu\nInvite me to skeet and neverlose :)";
...
function renderCheckbox(name, checkboxY, right, id, hint){
	...
	if(UI.IsCursorInBox(checkboxX, checkboxY, box_width - box_margin_x * 2, checkbox_height) && !slider_changing){
		if(id in script_items){
			...
			Render.Line(circle_x_start, circle_y_start + 1, circle_x_start, circle_y_start - 2, element_active_color);
			Render.Line(circle_x_start + 1, circle_y_start + 2, circle_x_start + 1, circle_y_start - 3, element_active_color);
			Render.Line(circle_x_start + 2, circle_y_start + 2, circle_x_start + 2, circle_y_start - 3, element_active_color);
			Render.Line(circle_x_start + 3, circle_y_start + 2, circle_x_start + 3, circle_y_start - 3, element_active_color);
			Render.Line(circle_x_start + 4, circle_y_start + 1, circle_x_start + 4, circle_y_start - 2, element_active_color);
